...
export function AnnotationSettingsPanel({
	...
	return (
		<div className="flex-[2] min-w-0 bg-cc-surface-0/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex flex-col shadow-2xl shadow-black/50 h-full overflow-y-auto custom-scrollbar">
			<div className="mb-6">
				<div className="flex items-center justify-between mb-4">
					<span className="text-sm font-medium text-slate-200">Annotation Settings</span>
					<span className="text-[10px] uppercase tracking-wider font-medium text-cc-accent bg-cc-accent/10 px-2 py-1 rounded-full">
						Active
					</span>
				</div>
				...
				>
					...
					<TabsContent value="text" className="mt-0 space-y-4">
						<div>
							...
							<textarea
								...
								rows={5}
								className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-slate-200 text-sm placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-cc-accent focus:border-transparent resize-none"
							/>
						</div>
							...
							<div className="grid grid-cols-2 gap-2">
								<div>
									...
									>
										...
										<SelectContent className="bg-cc-popover border-white/10 text-slate-200 max-h-[300px]">
								...
								<div>
									...
									>
										...
										<SelectContent className="bg-cc-popover border-white/10 text-slate-200 max-h-[200px]">
											{FONT_SIZES.map((size) => (
							...
							<div className="flex items-center justify-between gap-2">
								...
								>
									<ToggleGroupItem
										...
										className="h-8 w-8 data-[state=on]:bg-cc-accent data-[state=on]:text-white text-slate-400 hover:bg-white/5 hover:text-slate-200"
									>
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<Popover>
										...
										<PopoverContent className="w-[260px] p-3 bg-cc-popover border border-white/10 rounded-xl shadow-xl">
											<Block
					...
					<TabsContent value="image" className="mt-0 space-y-4">
						...
						<Button
							...
							variant="outline"
							className="w-full gap-2 bg-white/5 text-slate-200 border-white/10 hover:bg-cc-accent hover:text-white hover:border-cc-accent transition-all py-8"
						>
					...
					<TabsContent value="figure" className="mt-0 space-y-4">
						...
						<div>
							...
							<Popover>
								<PopoverTrigger asChild>
									...
									>
										<div
											...
											style={{ backgroundColor: annotation.figureData?.color || "#f97316" }}
										/>
										<span className="text-xs text-slate-300 truncate flex-1 text-left">
											{annotation.figureData?.color || "#f97316"}
										</span>
										<CaretDown size={12} weight="bold" className="opacity-50" />
									</Button>
								</PopoverTrigger>
								<PopoverContent className="w-[260px] p-3 bg-cc-popover border border-white/10 rounded-xl shadow-xl">
									<Block
										color={annotation.figureData?.color || "#f97316"}
										colors={colorPalette}
										onChange={(color) => {
											const newFigureData: FigureData = {
												...annotation.figureData!,
